...
function Navbar({ language, onLanguageChange }) {
  ...
  return (
    <header className="app-nav-wrap">
      <nav className={`app-nav${menuOpen ? ' menu-open' : ''}`}>
        ...
        <div className="app-nav-links">
          <NavLink className={({ isActive }) => `app-nav-link${isActive ? ' active' : ''}`} to="/" end onClick={closeMenu}>
            {copy.about}
          </NavLink>
          <NavLink className={({ isActive }) => `app-nav-link${isActive ? ' active' : ''}`} to="/partners" onClick={closeMenu}>
            {copy.partners}
          </NavLink>
          <NavLink className={({ isActive }) => `app-nav-link${isActive ? ' active' : ''}`} to="/calendar" onClick={closeMenu}>
            {copy.calendar}
          </NavLink>
          <NavLink className={({ isActive }) => `app-nav-link${isActive ? ' active' : ''}`} to="/contact" onClick={closeMenu}>
            {copy.contact}
          </NavLink>

          <div className="lang-switch" aria-label="Language selection">
            <select value={language} onChange={(event) => { onLanguageChange(event.target.value); closeMenu(); }}>
